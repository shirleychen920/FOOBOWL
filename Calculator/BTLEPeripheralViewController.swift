...
class BTLEPeripheralViewController: UIViewController, CBPeripheralManagerDelegate, UITextViewDelegate {
    ...
    @IBOutlet private weak var numOfPeoView: UITextView!

    @IBOutlet private weak var tipView: UITextView!
    ...
    func peripheralManager(peripheral: CBPeripheralManager, central: CBCentral, didSubscribeToCharacteristic characteristic: CBCharacteristic) {
        ...
        let numberOfPeople:Double=(NSString(string:numOfPeoView.text).doubleValue)
        let tip:Double=(NSString(string:tipView.text).doubleValue)
        
        
        // Get the data
        let data = (NSString(string: textView.text).doubleValue)/numberOfPeople*(tip/100+1)
        let each = NSString(format: "%.2f", data) as String
        let send = "$" + each + "\n" + "With a tip of "+tipView.text+"%"
    ...
    @IBAction func SendingSwitchChanged(sender: UISwitch) {
        if advertisingSwitch.on {
    ...
    @IBAction func ChangeStepper(sender: UIStepper) {
         ...
         tipView.text = Int(sender.value).description
    }
    
    @IBAction func ServiceSwitchChanged(sender: AnyObject) {
        if ServiceFeeSwitch.on {
            tipView.text = "0"
        } else {
             tipView.text = "15"
        }
